...
$sender_io->on('connection', function ($socket) {
    ...
    $socket->on('login', function ($uid) use ($socket) {
        ...
        $socket->uid = $uid;
        // 更新这个socket对应页面的在线数据
        $socket->emit('update_online_count', json_encode($uidConnectionMap) . "|||当前<b>{$last_online_count}</b>人在线，共打开<b>{$last_online_page_count}</b>个页面");
    });
...
$sender_io->on('workerStart', function () {
    // 监听一个http端口
    $inner_http_worker = new Worker('http://0.0.0.0:2124');
    // 当http客户端发来数据时触发
    $inner_http_worker->onMessage = function ($http_connection, $data) {
        ...
        switch (@$_POST['type']) {
            case 'publish':
                ...
                if ($to) {
                    $sender_io->to($to)->emit('new_msg', $uid . '|||' . $_POST['content']);
                    // 否则向所有uid推送数据
                } else {
...
//                echo $uid . "\n";
//                echo $_POST['content'] . "\n";
                return $http_connection->send($uidConnectionMap[$to]);
    ...
    Timer::add(1, function () {
        ...
        if ($last_online_count != $online_count_now || $last_online_page_count != $online_page_count_now) {
            $sender_io->emit('update_online_count', json_encode($uidConnectionMap) . "|||当前<b>{$online_count_now}</b>人在线，共打开<b>{$online_page_count_now}</b>个页面");
